...
public enum MemoryUpdateInterval {
    /// Minimum time between updates (12 hours = twice a day max)
    public static let minimumInterval: TimeInterval = 12 * 60 * 60
    ...
    public static let defaultMaxMemoryLength = 2000
    /// Minimum allowed memory limit
    public static let minMemoryLength = 500
    /// Maximum allowed memory limit
    public static let maxMemoryLength = 2000
}

/// Result of a scheduled memory update
public struct ScheduledMemoryUpdateResult: Equatable {
    public static func == (lhs: ScheduledMemoryUpdateResult, rhs: ScheduledMemoryUpdateResult) -> Bool {
        lhs.tier == rhs.tier &&
    ...
    public let tierId: UUID? // nil for global
    public let recordsProcessed: Int
    public let previousMemoryLength: Int
    public let newMemoryLength: Int
    public let success: Bool
    public let error: Error?
    public let skippedReason: SkipReason?
...
final class MemoryUpdateScheduler: ObservableObject {
    ...
    public init(settings: SharedSettings? = nil, providerFactory: ProviderFactory? = nil) {
    ...
    /// Called on app start
    public func performScheduledUpdates() async -> [ScheduledMemoryUpdateResult] {
    ...
    public func forceUpdateAll() async -> [ScheduledMemoryUpdateResult] {
        guard !isUpdating else { return [] }
...
public extension MemoryUpdateScheduler {
