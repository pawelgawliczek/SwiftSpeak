...
public struct TokenCounter {
    ...
    public static func estimatePrecise(_ text: String) -> Int {
        guard !text.isEmpty else { return 0 }
    ...
    public static func inputLimit(for model: String, outputBuffer: Int = 1000) -> Int {
    ...
    public static func truncate(_ text: String, maxTokens: Int, suffix: String = "...") -> String {
    ...
    public static func truncateSmart(_ text: String, maxTokens: Int) -> String {
        let estimatedTokens = estimate(text)
...
public struct PromptLimits {

    /// Global memory section
    public static let globalMemory = 500

    /// Context-specific memory
    public static let contextMemory = 400

    /// Power mode memory
    public static let powerModeMemory = 300

    /// RAG document chunks
    public static let ragChunks = 2000

    /// Webhook context data
    public static let webhookContext = 1000

    /// User transcription input
    public static let userInput = 4000

    /// Custom template prompt
    public static let customTemplate = 500

    /// Vocabulary hints
    public static let vocabularyHint = 200

    /// Total prompt limit (conservative default)
    public static let totalPrompt = 8000

    /// System prompt base (before user content)
    public static let systemPromptBase = 1000
    ...
    public static func availableForUser(model: String) -> Int {
    ...
    public static func availableForRAG(model: String) -> Int {
        let modelLimit = TokenCounter.inputLimit(for: model)
...
public struct TokenBudget {
    ...
    /// Remaining tokens
    public var remaining: Int { total - used }

    /// Whether budget is exhausted
    public var isExhausted: Bool { used >= total }

    /// Percentage used (0-100)
    public var percentUsed: Int { total > 0 ? min(100, (used * 100) / total) : 0 }

    public init(total: Int) {
        self.total = max(0, total)
    }

    public init(for model: String) {
    ...
    public mutating func allocate(_ count: Int) -> Bool {
    ...
    public mutating func forceAllocate(_ count: Int) {
        used += count
    }
    ...
    public mutating func allocateText(_ text: String, maxPortion: Double = 1.0) -> String {
    ...
    public func canFit(_ text: String) -> Bool {
