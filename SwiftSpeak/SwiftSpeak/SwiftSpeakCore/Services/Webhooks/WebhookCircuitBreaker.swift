...
public actor WebhookCircuitBreaker {
    ...
    public func shouldExecute(_ webhookId: UUID) -> Bool {
    ...
    public func recordSuccess(_ webhookId: UUID, latency: TimeInterval) {
    ...
    public func recordFailure(_ webhookId: UUID, error: String? = nil) {
        var webhookStats = stats[webhookId] ?? WebhookStats()
    ...
    public func recordTimeout(_ webhookId: UUID) {
    ...
    public func getState(_ webhookId: UUID) -> CircuitState {
        stats[webhookId]?.circuitState ?? .closed
    }

    /// Get stats for a webhook
    public func getStats(_ webhookId: UUID) -> WebhookStats {
        stats[webhookId] ?? WebhookStats()
    }

    /// Get all webhook stats
    public func getAllStats() -> [UUID: WebhookStats] {
        stats
    }

    /// Check if any webhooks are in open state
    public func hasOpenCircuits() -> Bool {
        stats.values.contains { $0.circuitState == .open }
    }

    /// Get list of open circuit webhook IDs
    public func openCircuitIds() -> [UUID] {
    ...
    public func reset(_ webhookId: UUID) {
    ...
    public func forceClose(_ webhookId: UUID) {
    ...
    public func forceOpen(_ webhookId: UUID) {
        var webhookStats = stats[webhookId] ?? WebhookStats()
        webhookStats.setCircuitState(.open)
        stats[webhookId] = webhookStats
        circuitOpenedAt[webhookId] = Date()
...
public extension WebhookCircuitBreaker {
...
public extension WebhookCircuitBreaker {
    ...
    public static let shared = WebhookCircuitBreaker()
}
