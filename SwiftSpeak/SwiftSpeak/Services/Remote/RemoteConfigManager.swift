...
final class RemoteConfigManager: ObservableObject {
    ...
    func fetchConfigIfNeeded(force: Bool = false) async {
    ...
    func providerConfig(for provider: AIProvider) -> ProviderRemoteConfig? {
        config?.providers[provider.rawValue]
    }

    /// Get pricing for a specific model
    func pricing(for provider: AIProvider, model: String) -> PricingRemoteConfig? {
    ...
    func providerStatus(_ provider: AIProvider) -> ProviderOperationalStatus {
...
/*
 ...
 4. Update fetchFromRemote() to use:
    ...
    let jsonString = remoteConfig.configValue(forKey: "provider_config").stringValue
