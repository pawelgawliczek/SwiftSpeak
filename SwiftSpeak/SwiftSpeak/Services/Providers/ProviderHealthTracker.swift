...
actor ProviderHealthTracker {
    ...
    func recordFailure(for provider: AIProvider, error: String? = nil) {
        var health = healthStatus[provider] ?? ProviderHealth()
    ...
    func getHealth(for provider: AIProvider) -> ProviderHealth {
        healthStatus[provider] ?? ProviderHealth()
    }

    /// Get all provider health statuses
    func getAllHealth() -> [AIProvider: ProviderHealth] {
        healthStatus
    }

    /// Reset health tracking for a provider
    func reset(for provider: AIProvider) {
    ...
    func rankProviders(_ providers: [AIProvider]) -> [AIProvider] {
...
extension ProviderHealthTracker {
    /// Check health and return reason if unhealthy
    func healthCheck(_ provider: AIProvider) -> (healthy: Bool, reason: String?) {
