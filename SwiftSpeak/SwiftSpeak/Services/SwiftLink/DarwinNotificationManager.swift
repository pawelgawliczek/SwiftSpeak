...
//
//  SHARED: This file should be copied to SwiftSpeakKeyboard target
//
...
// MARK: - SwiftLink Convenience Methods
...
extension DarwinNotificationManager {
    ...
    /// Post that transcription result is ready (from main app)
    func postResultReady() {
        post(name: Constants.SwiftLinkNotifications.resultReady)
    }

    /// Post that SwiftLink session has started (from main app)
    func postSessionStarted() {
        post(name: Constants.SwiftLinkNotifications.sessionStarted)
    }

    /// Post that SwiftLink session has ended (from main app)
    func postSessionEnded() {
        post(name: Constants.SwiftLinkNotifications.sessionEnded)
    }

    /// Observe dictation start (in main app)
    func observeDictationStart(callback: @escaping () -> Void) {
        startObserving(name: Constants.SwiftLinkNotifications.startDictation, callback: callback)
    }

    /// Observe dictation stop (in main app)
    func observeDictationStop(callback: @escaping () -> Void) {
        startObserving(name: Constants.SwiftLinkNotifications.stopDictation, callback: callback)
    }
