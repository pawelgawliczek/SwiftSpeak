...
//  SwiftSpeakMac
//
//  UI component for configuring Power Mode hotkeys
//  Allows users to record key combinations and assign them to Power Modes
//
...
struct MacPowerModeHotkeyEditor: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // Info Text
            Text("Global hotkey to activate this Power Mode from anywhere")
                .font(.caption)
                .foregroundColor(.secondary)
        }
    ...
    private func isHotkeyInUse(keyCode: UInt16, modifiers: UInt) -> Bool {
        // Check if this exact combination is used by another Power Mode
        for (id, combo) in settings.powerModeHotkeys {
            if id != powerMode.id && combo.keyCode == keyCode && combo.modifiers == modifiers {
                return true
            }
        }
...
#if DEBUG
struct MacPowerModeHotkeyEditor_Previews: PreviewProvider {
    static var previews: some View {
        MacPowerModeHotkeyEditor(
            powerMode: PowerMode(
                id: UUID(),
                name: "Sample Mode",
                icon: "star.fill",
