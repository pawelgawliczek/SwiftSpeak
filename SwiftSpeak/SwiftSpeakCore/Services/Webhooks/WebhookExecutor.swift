...
final class WebhookExecutor {
    ...
    public init(settings: SharedSettings, session: URLSession? = nil) {
    ...
    public func fetchContext(for powerMode: PowerMode) async -> [ContextSourceResult] {
    ...
    public func triggerAutomations(
        for powerMode: PowerMode,
        input: String,
        output: String,
        contextName: String?
    ...
    public func testWebhook(_ webhook: Webhook) async -> WebhookResult {
        let startTime = Date()
