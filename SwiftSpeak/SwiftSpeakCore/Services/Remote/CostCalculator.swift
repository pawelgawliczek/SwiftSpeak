...
public struct CostCalculator {
    ...
    public init(configManager: RemoteConfigManager) {
    ...
    public func calculatePredictionCostBreakdown(
        provider: AIProvider,
        model: String,
        inputTokens: Int,
        outputTokens: Int,
    ...
    public func estimateMonthlyUsageCost(
    ...
    public func compareCostsPerMinute(
...
public extension CostCalculator {
    ...
    public func estimateTypicalDictationCost(provider: AIProvider, model: String) -> Double {
        transcriptionCost(provider: provider, model: model, durationSeconds: 30)
    }

    /// Get the cheapest provider for a capability
    public func cheapestProvider(for capability: ProviderUsageCategory) -> AIProvider? {
