...
public struct SentencePredictionContext: Codable, Equatable {
    /// The typing context that was used for prediction
    public let typingContext: String
    ...
    public let activeContextName: String?

    public init(typingContext: String, prompt: String, predictions: [String], activeContextName: String? = nil) {
...
public struct EditContext: Codable, Equatable {
    ...
    public init(originalText: String, instructions: String, parentEntryId: UUID? = nil) {
...
public struct TranscriptionRecord: Identifiable {
    public let id: UUID

    // Text content - Phase 11: Store both raw and final
    public let rawTranscribedText: String  // Raw text before formatting (new)
    public let text: String                // Final formatted/translated text
    ...
    public let powerModeId: UUID?       // nil for regular transcriptions
    public let powerModeName: String?   // Cached name for display even if mode deleted
    public let contextId: UUID?         // nil if no context was active
    public let contextName: String?     // Cached name for display even if context deleted
    public let contextIcon: String?     // Cached icon for display

    // Cost tracking (Phase 9)
    public let estimatedCost: Double?   // Total estimated cost for this transcription
    public let costBreakdown: CostBreakdown?  // Detailed cost breakdown by operation type
    ...
    public let sentencePredictionContext: SentencePredictionContext?
    ...
    public let globalMemoryEnabled: Bool
    public let contextMemoryEnabled: Bool
    public let powerModeMemoryEnabled: Bool
...
public extension TranscriptionRecord: Codable {
